...
class BST<K extends Comparable<K>, E> implements Dictionary<K, E> {
  ...
  /**
   * Creates a list storing the the nodes in the subtree of a node, ordered
   * according to the levelorder traversal of the subtree.
   */

  protected void levelorderElements(BSTNode<K, E> v, ArrayList<E> elts) {
    ArrayDeque<BSTNode> nodeQueue = new ArrayDeque<BSTNode>();
    nodeQueue.add(v); // Add root to queue
    while(nodeQueue.peek() != null){ //While queue is not empty
      int levelSize = nodeQueue.size(); //number of nodes in current level
      for(int i = 0; i < levelSize; i++){
        BSTNode curr = nodeQueue.poll(); //dequeue
          elts.add((E) curr.element()); //add the element to the list
          if (curr.left() != null)
            nodeQueue.add(curr.left()); //add left child to queue
          if (curr.right() != null)
            nodeQueue.add(curr.right()); //add right child to queue
      }
    }
  }
  ...
  public Iterable<E> values() {
    ...
    if (size() != 0)
      levelorderElements(root, elements); // assign positions in level order
    return elements;
  }
